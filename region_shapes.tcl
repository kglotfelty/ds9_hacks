...
ttk::frame $ds9(main).quick.shapes
grid $ds9(main).quick.shapes -row 0 -column $atcol -columnspan 1 -padx 5
incr atcol

ttk::button $ds9(main).quick.shapes.circle -text {C} -takefocus 0 -command {my_change_marker circle} \
    -image [image create photo -file "$ds9_hack_root/UI/circle.png"] 
ttk::button $ds9(main).quick.shapes.annulus -text {A} -takefocus 0 -command {my_change_marker annulus} \
    -image [image create photo -file "$ds9_hack_root/UI/annulus.png"]
ttk::button $ds9(main).quick.shapes.ellipse -text {E} -takefocus 0 -command {my_change_marker ellipse} \
    -image [image create photo -file "$ds9_hack_root/UI/ellipse.png"]
ttk::button $ds9(main).quick.shapes.box -text {B} -takefocus 0 -command {my_change_marker box} \
    -image [image create photo -file "$ds9_hack_root/UI/box.png"]
ttk::button $ds9(main).quick.shapes.poly -text {P} -takefocus 0 -command {my_change_marker polygon} \
    -image [image create photo -file "$ds9_hack_root/UI/poly.png"]
ttk::button $ds9(main).quick.shapes.text -text {T} -takefocus 0 -command {my_change_marker text} \
